...
## v6.2 신규: 업체 고정 배정 (Pinned Companies)

사용자가 특정 업체를 특정 날짜에 **미리 고정**할 수 있습니다.
...
1. "📌 고정 모드" 버튼 클릭
2. 고정할 날짜 선택
3. 업체 목록에서 업체 클릭 → 해당 날짜에 고정
4. "스케줄 생성" 클릭 → 고정 업체가 먼저 배정됨
```
...
| 순서 | 설명 |
|------|------|
| 1 | 고정 인덱스 구축 (`rebuildPinIndex`) |
| 2 | 고정 업체를 해당 날짜에 먼저 배정 (`applyPinsToSchedule`) |
| 3 | 고정 업체를 후보 풀에서 제외 |
| 4 | 나머지 업체에 대해 기존 알고리즘 실행 |

### 데이터 구조

```javascript
// state.pinnedCompanies
[{ companyId: 123, date: "2026-01-15", companyName: "남경스틸" }]

// state.pinnedByCompany (Map)
companyId -> date

// state.pinnedByDate (Map)
date -> Set<companyId>
```
...
- **v6.2**: `pinnedCompanies`도 함께 저장/복원
...
---
*최종 업데이트: 2026-01-10 (v6.2 고정 배정 기능 추가)*
