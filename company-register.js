...
function formatAddress(rawAddress) {
    ...
    // 4. 시/군/구 뒤에 읍/면/동 앞에 공백 추가
    // "김해시생림면" -> "김해시 생림면"
    addr = addr.replace(/([시군구])([가-힣]+[읍면동])/, '$1 $2');

    // 5. 읍/면/동 뒤에 도로명(대로/로/길) 앞에 공백 추가
    // "생림면생림대로" -> "생림면 생림대로"
    addr = addr.replace(/([읍면동])([가-힣]+(대로|로|길))/, '$1 $2');
